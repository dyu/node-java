...
var rp = require('./runtime_path');
process.env += rp.java_require('build/jvm_dll_path.json')

var bindings = rp.java_require('build/Release/nodejavabridge_bindings.node')
...
if (rp.hasOwnProperty('java_init')) {
    rp.java_init(java)
} else {
    java.classpath.push(rp.java_resolve('commons-lang3-node-java.jar'));
    java.classpath.push(rp.java_resolve('src-java'));
    java.classpath.pushDir = function(dir) {
        fs.readdirSync(dir).forEach(function(file) {
            java.classpath.push(path.resolve(dir, file));
        });
    };
    java.nativeBindingLocation = rp.java_resolve('build/Release/nodejavabridge_bindings.node');
}
